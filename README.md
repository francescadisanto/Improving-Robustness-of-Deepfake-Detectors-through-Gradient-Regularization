...
To enhance generalization and improve robustness against adversarial attacks, we implement a gradient-inspired regularization strategy following the idea of Guan et al. (2024).

This method specifically addresses a known limitation of deepfake detectors: their tendency to overfit to superficial texture cues, leading to poor generalization on unseen manipulation sources.

Instead of explicitly computing gradients with respect to the model’s weights or shallow features, our method directly perturbs the feature statistics—mean (µ) and standard deviation (σ)—in the early convolutional layers.
This stochastic perturbation simulates gradient regularization, forcing the model to learn representations that are less sensitive to texture artifacts and more robust to distribution shifts.
