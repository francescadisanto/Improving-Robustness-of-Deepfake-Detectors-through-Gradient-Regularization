...
This method specifically addresses a known limitation of deepfake detectors: their tendency to overfit to superficial texture cues, which lead to poor generalization on unseen fake sources.
...
In addition to regularization, we experimented with adversarial training, which injects adversarial examples into the training process to further boost robustness. We generate attacks using **FGSM** during training with a small epsilon and include them in the loss computation alongside clean samples:
```python
loss_adv = (1 - beta) * loss_clean + beta * loss_adv_example
